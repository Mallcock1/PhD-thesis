...
All models are wrong. What's more, we don't even want them to be right. They are wrong by design. Mathematical models simplify real phenomena and are constructed to contain precisely the essential components. That way, mathematical results about the model can be gained which can provide insights into the inner workings of the physical system that is being modelled.

This thesis is about mathematical modelling of phenomena in the atmosphere of the Sun. In solar physics, mathematical modelling is one of the three major cornerstones that contribute to scientific progress, the other two being observations and numerical simulations. The process of mathematical modelling in solar physics is as follows. First, we observe solar phenomena, with all their noise and rough edges. Next, we conceptually strip off the noise, smooth the edges, and simplify the geometry until we are left with a model with only the essential features of the original system. Using the arsenal of mathematical tools at our disposal, we can derive results about the model. Finally, these results are mapped back onto the original phenomenon, giving us insights about the real world. It's a mathematical expedition to analyse a physical system.

But not all mathematical expeditions are \textit{worthwhile}\footnote{This is the key distinction between pure and applied mathematics. Pure mathematics is defined by taking mathematical expeditions for their own sake.}. Informally, we might say that a mathematical model, $M$, of a physical system, $X$, is worthwhile if it satisfies two conditions:
\begin{enumerate}
	\item Studying $M$ provides additional explanatory power of $X$ beyond what is possible to gain from analysing $X$ directly,
	\item $M$ approximates $X$ well. \label{approx}
\end{enumerate}
A model that provides additional explanatory power but does not approximate the real system well will provide useless explanations. A model that approximates the real system well but provides no explanatory power is merely art. Neither of which are worthwhile when the aim is to \textit{explain} the world.

The most significant obstacle impeding explanatory power is mathematical tractability. Mathematical theory is too blunt a weapon to attack complex models. So we are faced with a trade-off between simplicity and mathematical tractability. A model too simple will produce results that fail to map to reality. A model too complex will lose any hope of mathematical tractability. A fine balance is sought.

Unfortunately, it seems like this balance is getting harder and harder to find. With rapidly advancing instrumentation, solar observations are resolving features at unprecedented length and time-scales. This has shown us just how complex the solar atmosphere is, and, therefore, how complex our models need to be in order to capture the essential physics. The advancement of solar telescope instrumentation is vastly outpacing the development of new mathematical techniques that can pull more complex models into the land of tractability. So it is harder than ever to satisfy both criteria for worthwhile mathematical modelling. Here are two reasons why all hope is not lost quite yet:
\begin{enumerate}
	\item Alongside rapid advancements in solar instrumentation are rapid advancements in numerical simulations.
	\item Advancements in mathematical modelling may come in discontinuous jumps.
\end{enumerate}
Let's unpack these happenings. Numerical simulations of solar phenomena allow us to analyse more complex models. But this comes at a cost. while it is easier to reproduce physical phenomena using numerical simulations than mathematical models, it is very difficult to diagnose \textit{why} it happens. This is compounded by the possibility of multiple numerical set-ups reproducing the physical phenomenon, so that it can be unclear which one maps best to reality. We are witnessing an explosion in computational capability as Moore's law\footnote{Moore's law is the observation that the number of transistors in an integrated circuit follows an exponential increase, doubling approximately every two years.} is, against all predictions, still holding true. Therefore, we can expect numerical simulations to maintain a key role in solar physics. In this thesis, where they exists, we have included reference to relevant numerical simulations.

It is easy to think that mathematical theory progresses in a linear fashion, where each new mathematician produces an $\epsilon$ of progress, standing on the shoulders of those before. But does progress really look like this? Philosopher of science Thomas Kuhn argues that as science progresses, explanations tend to become more complex before a paradigm shift offers radical simplification. Such a paradigm shift could vastly expand the set of mathematical models that are tractable. Are we due a paradigm shift in mathematical modelling theory?

We don't expect this thesis to provide such a thing. Instead, we apply advanced, but known techniques to new mathematical models, with the aim of introducing novel techniques that can be used by observational solar physicists to better understand our star. All the models in this thesis are wrong. I'll try to convince you that some of them are useful.


\color{red}
Maybe include:

In addition, we need to establish a preference between models that score equally on these criteria. The natural choice is to introduce a principle of parsimony. Occam's Razor states that among models with equal explanatory power, the simplest one is the most desirable.

A project of this scope could do what category theory is doing to pure mathematics. 
\color{black}
