...
where
\begin{align}
	D_s(\omega) &= \rho_em_i(k^2v_\textrm{Ae}^2 - \omega^2)\tanh{m_ix_0} + \rho_im_e(k^2v_\textrm{Ai}^2 - \omega^2), \\
	D_k(\omega) &= \rho_em_i(k^2v_\textrm{Ae}^2 - \omega^2)\coth{m_ix_0} + \rho_im_e(k^2v_\textrm{Ai}^2 - \omega^2),
\end{align}
where $m_i$ and $m_e$ are defined in the fashion equivalent to Equation~\eqref{gov eqn}. The subscript $s$ and $k$ in the above dispersion functions refer to the ``sausage" and ``kink" modes that the eigenmodes can be categorised into depending on whether each of the interfaces oscillate in anti-phase or in phase, respectively.
...
\end{equation}
Outside of the tube, an equivalent equation, with subscripts $e$ is satisfied. For $m_i^2 > 0$, this is the modified Bessel's equation of order $m$ and for $m_i^2 > 0$, it is Bessel's equation of order $m$. Requiring that the perturbations approach zero far from the tube outside means that for $r > r_0$,
\begin{equation}
	p_T(r) = A K_m(m_e r),
\end{equation}
...
Given that each kind of the modified Bessel functions of integer order, when considered as functions of their order, are even functions (\textit{i.e.} $I_{-n} (z) = I_n(z)$ and $K_{-n} (z) = K_n(z)$), modes with negative orders are identical modes to their positive order counterparts.
...
\label{DRsym}
\end{align}
The subscript $S$ and $k$ in the above dispersion functions refer to the ``sausage" and ``kink" modes that the eigenmodes can be categorised into depending on whether each of the interfaces oscillate in anti-phase or in phase, respectively.
