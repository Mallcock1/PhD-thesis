...
\begin{equation}
	\frac{\omega^2}{k^2} = \frac{\rho_+m_-v_{A+}^2 + \rho_-m_+v_{A-}^2}{\rho_+m_- + \rho_-m_+}, \quad \text{where} \quad m_\pm^2 = \frac{(k^2v_{A\pm}^2 - \omega^2)(k^2c_\pm^2 - \omega^2)}{(c_\pm^2 + v_{A\pm}^2)(k^2c_{T\pm}^2 - \omega^2)}.
\end{equation}
...
\end{cases}
\end{equation}
By Fourier decomposing into the form $f(\mathbf{r}) = \hat{f}(r)e^{i()}$ for each parameter, inside the tube the radial velocity perturbation obeys the equation

\textcolor{red}{Try to get a nice explanation for getting to cylindrical governing ODE}
