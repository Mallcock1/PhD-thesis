...
All models are wrong. And we don't even want them to be right. They are wrong by design. Mathematical models are simplifications of real phenomena and are constructed to contain precisely the essential components.

Mathematical modelling sits at the core of applied mathematics. First, we take structures in the real world, with all their noise and spike bits. Next, we strip off the noise, smooth the edges, and simplify the geometry until we are left with a model with only the essential features. Using the arsenal of mathematical tools at our disposal, we can derive results about the model. Finally, these results are mapped back onto the original object. It's a mathematical expedition to analyse a physical system.

But not all mathematical expeditions are worthwhile\footnote{This is the key distinction between pure and applied mathematics. Pure mathematics is defined by taking mathematical expeditions for their own sake.}. A mathematical model, $M$, of a physical system, $X$, is worthwhile if it satisfies two conditions:
\begin{enumerate}
	\item Studying $M$ provides additional explanatory power of $X$ beyond what is possible to gain from analysing $X$ itself,
...
In addition to these criteria, we need to establish a preference between models that score equally on these criteria. The natural choice is that of parsimony. Occam's Razor states that among models with equal explanatory power, the simplest one is the most desirable.

For mathematical modelling in particular, the most significant obstacle impeding explanatory power is mathematical tractability. Mathematical theory is too blunt a weapon to attack complex models. So we are faced with a trade-off between simplicity and mathematical tractability. A model too simple will produce results that fail to map to reality. A model too complex will lose any hope of mathematical tractability. A fine balance is sought.

\color{red}
This is a trade-off that gets more difficult to navigate as the pace of applying known mathematical techniques to simple models vastly outpaces the development of new mathematical techniques that can pull more complex models into the land of tractability.
...
All the models in this thesis are wrong. I'll try to convince you that some of them are useful.
\color{black}
