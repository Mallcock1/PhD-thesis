...
To study the time-dependent behaviour of the transverse velocity, we start by studying the asymptotic behaviours of $A_1(t) = v_x(-x_0, t)$ and $A_2(t) = v_x(x_0, t)$. These variables are related to the Laplace transformed variables $\tilde{A}_1$ and $\tilde{A}_2$ by the inverse Laplace transform (non-standard, discussed in Appendix~\ref{app: laplace trans}), namely
\begin{equation}
...
So the integrand has 18 branch points and an infinite number of poles that are not possible to describe analytically. This is an intractable problem. Therefore, we will instead solve the simplified problem of a thin symmetric slab with zero-beta plasma. From there, we will study what would happen when symmetry is broken.

\subsubsection{Solving a simplified case - Thin zero-beta symmetric slab}

When simplifying to a symmetric slab, we use the notation subscript $e$ to denote the symmetric \textit{external} environment, rather than subscripts 1 and 2. Under the zero-beta approximation, the tube speed is identical to the sound speed. Therefore, the points $\omega = \pm \omega_{00,e}, \omega_{T0,e}$ are no longer branch points. The only remaining branch points are $\omega = \pm \omega_{A0,e}$
...
\begin{figure}
	...
	\scalebox{0.9}{
		...
		\draw[fill, blue] (-1.25,0) circle [radius=0.1];
