...
The ability to diagnose multiple parameters simultaneously could get around the hurdle of uncertain density measurements. However, the cost of this is that by estimating several parameters, we are more likely to encounter multiple roots. 

To my knowledge, this is the first time that using a combination of SMS techniques to simultaneously diagnose multiple parameters has been proposed.



	
	
	My approach to this problem utilises a version of \textit{Powell's Hybrid method} for optimisation, modified for finding roots of a system of multivariate functions. This is particularly valuable because, although it is the magnetic field strength that is traditionally thought of as being impossible to measure, measurements of the density in solar structures also come with significant uncertainties. 

\subsubsection{Error analysis}

From 2 year report:
Every measurement comes with error, and when assessing the efficacy of a new measurement technique, an analysis of the errors must be undertaken. There are two kinds of error in a diagnosis made using the AR and MPS methods: \textit{propagated input errors} that are due to the measurement errors of the input parameters, and \textit{systematic errors} that are due to the model fitting the real structure less than perfectly. We do not currently know of any way to quantify the systematic errors. However, it is possible to make an estimation of the propagated input errors. Because of the low sensitivity of density sensitive absorption lines in the observed light spectra, the largest input error is in the measurement of the background density. I found that the contribution of this error to that of the magnetic field diagnosis after propagation through the inversion is a half of the original error in the density measurement. This is on par with the best previous methods (\textit{e.g.} \citealt{nak_etal01}).
...
\subsubsection{Dealing with multiple roots}
