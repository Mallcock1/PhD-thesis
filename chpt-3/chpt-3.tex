...
\end{equation}
where
\begin{equation}f(x) = ik\left\{\rho_0[\dot{\hat{\Omega}}_0 - i\omega\hat{\Omega}_0] + \frac{d\rho_0}{dx}(\dot{\hat{v}}_{z0} - i\omega\hat{v}_{\textcolor{red}{z}0})\right\}.
\end{equation}
This function is the corrected version of Equations~(11)-(13) of \cite{rae_etal81}. A typographical error was made in the above equation, where they wrote subscript $x$ in place of our subscript $\textcolor{red}{z}$. The factor of -1 discrepency between this function and that of \cite{rae_etal81} is due to taking different Fourier forms.
...
& = -\frac{v_0}{\pi z_0} \int_{0}^{\infty} \frac{1}{k} [\sin(k(z + z_0)) - \sin(k(z - z_0))] [(\cos(kv_{A\pm}t) - \cos(kv_st))e^{-k|x|} - \cos(kv_{A\pm}t)] ~dk,
\end{align}
...
\begin{align}
&\rho_0(k^2v_{A0}^2 - \omega^2)[\rho_1(k^2v_{A1}^2 - \omega^2) + \rho_2(k^2v_{A2}^2 - \omega^2)] \notag \\
&+ [\rho_0^2(k^2v_{A0}^2 - \omega^2)^2 + \rho_1\rho_2(k^2v_{A1}^2 - \omega^2)(k^2v_{A2}^2 - \omega^2)]\tanh(2kx_0) = 0.
\end{align}
...
The time-dependant evolution of a perturbation of an incompressible asymmetric magnetic slab are thus 
