...
\begin{figure}[]
	\centering
	\subfloat[$v_{A0} > c_0$]{\includegraphics[scale=0.45]{../main/figures/disp-mag-outside-photophere.png}
		\label{fig: disp mag 1}}
	\subfloat[$v_{A0} < c_0$]{\includegraphics[scale=0.45]{../main/figures/disp-mag-outside-2.png}
...
%	\label{tab: eigenmode differences}
%\end{table}
A second implication that asymmetric eigenmodes have on mode identification in solar observations is that a waveguide oscillating in an asymmetric mode could be indistinguishable from one oscillating in a superposition of symmetric eigenmodes.
...
To appreciate the potential misidentification, consider an observation of a waveguide in the solar atmosphere from which oscillations can be identified in both the cross-sectional width and the axis (for example, Figure~\ref{}, based on the same dataset at was analysed by \cite{mor12}). The naive approach is to assume symmetry of the waveguide, so that the only explanation for the 





This is due to the mixed properties of asymmetric eigenmodes i


\textcolor{red}{Table of observable indicators of each case.}
