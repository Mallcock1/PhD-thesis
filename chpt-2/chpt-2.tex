...
\textcolor{red}{need to go through and check/change/reference prev sections fro this bit}

There is a rich spectrum of MHD waves supported by a magnetic slab. In a symmetric slab, the dispersion relation, Equation~\eqref{DRsym}, consists of two decoupled equations that correspond to the two types of fundamental wave supported by the slab: the ``sausage" and ``kink" MHD waves. Sausage and kink modes have been observed, for example, in chromospheric fibrils, where the ubiquity of these waves has been linked to coronal heating \citep{mor12}.
