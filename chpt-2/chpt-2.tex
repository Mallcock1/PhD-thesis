...
\end{equation}
for $j = 1, 2$, respectively. This equation is the same as the dispersion relation governing surface waves along a single interface between a magnetized and a non-magnetized plasma, Equation\eqref{DR interface}. Hence, the surface mode solutions of a wide asymmetric slab are just the surface modes that propagate along each interface independently. This corroborates our intuition that as the slab is widened the interfaces will have diminishing influence on each other, with the limit of no influence at all, in which case each interface can oscillate independently with its own characteristic frequency.

Unfortunately, the body waves have no parallel in the single interface model because body waves in a slab that are externally evanescent owe their existence to the two interfaces. For body modes in the wide slab limit, there exists a solution that behaves like $\omega^2 \to k^2c_0^2$ as $kx_0 \to \infty$. To see this, substitute the ansatz $\omega^2 = k^2c_0^2\left(1 + \nu/(kx_0)^2\right)$ into the dispersion relation, Equation~\eqref{DR approx non-mag}, to retrieve the family of quasi-sausage body modes given by
\begin{equation}
\omega^2 = k^2c_0^2\left(1 - \frac{\pi^2(j - \frac{1}{2})^2c_0^2}{(v_\textrm{A0}^2 - c_0^2)(kx_0)^2}\right), \quad j = 1, 2, \ldots \label{wide slab saus body}
\end{equation}
in the wide slab limit. Similarly, there exist quasi-kink body mode solutions given by
\begin{equation}
\omega^2 = k^2c_0^2\left(1 - \frac{\pi^2j^2c_0^2}{(v_\textrm{A0}^2 - c_0^2)(kx_0)^2}\right), \quad j = 1, 2, \ldots \label{wide slab kink body}
\end{equation}
in the wide slab limit. These are valid solutions only when $v_\textrm{A0} > c_0$.

This analysis may then be repeated for $v_\textrm{A} < c_0$ to find that, in the wide slab limit, there exist quasi-sausage body mode solutions given by
\begin{equation}
\omega^2 = k^2v_\textrm{A0}^2\left(1 - \frac{\pi^2(j - \frac{1}{2})^2v_\textrm{A0}^2}{(c_0^2 - v_\textrm{A0}^2)(kx_0)^2}\right), \quad \text{for} \quad j = 1, 2, \ldots \label{wide slab saus body2}
\end{equation}
and quasi-kink body mode solutions of the form
\begin{equation}
\omega^2 = k^2v_\textrm{A0}^2\left(1 - \frac{\pi^2j^2v_\textrm{A0}^2}{(c_0^2 - v_\textrm{A0}^2)(kx_0)^2}\right), \quad \text{for} \quad j = 1, 2, \ldots. \label{wide slab kink body2}
\end{equation}
These solutions to the approximate asymmetric slab dispersion relation demonstrate that to quadratic order of $1/kx_0$, the wide slab body modes are independent of the external plasma parameters. Therefore, Equations~\eqref{wide slab saus body}, \eqref{wide slab kink body}, \eqref{wide slab saus body2}, and \eqref{wide slab kink body2} are identical to the body mode solutions in a wide symmetric slab \citep{rob81b}. Equations~\eqref{thin slab slow saus surf}-\eqref{thin slab kink body},~\eqref{wide slab saus body} and~\eqref{wide slab kink body} also appear in~\cite{li_etal13} for a symmetric magnetic slab with shear flow when the shear flow speed is set to zero.


\subsection{Numerical solutions} \label{sec: numerical solutions}
\subsubsection{Dispersion diagrams} \label{sec: disp diagrams}

By solving the dispersion relation numerically, it can be shown that although the fast quasi-sausage surface mode does not exist in the thin slab limit for an asymmetric slab, it can exist in the wide slab limit (\textit{i.e.} as a solution to Equation~\eqref{DRwide}) when $c_0 < v_\textrm{A0}$, $c_1 < v_\textrm{A0}$, and $c_2 < v_\textrm{A0}$. For example, Figure~\ref{fig:disp sbs} shows that the minimum of $c_1$ and $c_2$ becomes a new cut-off, causing the fast quasi-sausage surface mode to degenerate for small $kx_0$.

%\begin{figure}[]
%	\centering
%	\subfloat[$v_\textrm{A}>c_0$]{\includegraphics[scale=0.45]{disp_sbs_R1-5.pdf}
%		\label{fig:disp sbs}}
%	\subfloat[$v_\textrm{A}<c_0$]{\includegraphics[scale=0.45]{disp_sbb_R1-5.pdf}
%		\label{fig:disp sbb}}
%	\caption{Dispersion diagram for the dispersion relation, Equation~\eqref{DR}. The surface (body) modes are in plotted red (blue) and the sausage (kink) modes are represented by solid (dashed) lines. The density ratios are $\rho_1/\rho_0 = 1.5$ and $\rho_2/\rho_0 = 2$, and the characteristic speed orderings are $c_2=1.2c_0$ and (a)$v_\textrm{A}=1.3c_0$ and (b)$v_\textrm{A}=0.9c_0$.}
%\end{figure}
